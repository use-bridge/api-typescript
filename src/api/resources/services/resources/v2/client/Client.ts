...
export class V2 {
     ...
     *
     * @param {string} id
     * @param {BridgeApi.services.ServiceUpdateV2Request} request
     * @param {V2.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @example
    ...
    public updateService(
        id: string,
        request: BridgeApi.services.ServiceUpdateV2Request,
        requestOptions?: V2.RequestOptions,
    ): core.HttpResponsePromise<BridgeApi.services.ServiceUpdateV2Response> {
    ...
    private async __updateService(
        ...
        request: BridgeApi.services.ServiceUpdateV2Request,
        requestOptions?: V2.RequestOptions,
    ): Promise<core.WithRawResponse<BridgeApi.services.ServiceUpdateV2Response>> {
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            this._options?.headers,
            mergeOnlyDefinedHeaders({ ...(await this._getCustomAuthorizationHeaders()) }),
            requestOptions?.headers,
        ...
        const _response = await core.fetcher({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.BridgeApiEnvironment.Production,
                `/api/services/v2/${encodeURIComponent(id)}`,
            ),
            method: "POST",
            headers: _headers,
            contentType: "application/json",
        ...
        if (_response.ok) {
            return {
                data: _response.body as BridgeApi.services.ServiceUpdateV2Response,
                rawResponse: _response.rawResponse,
            };
        }
        ...
        switch (_response.error.reason) {
            ...
            case "timeout":
                throw new errors.BridgeApiTimeoutError("Timeout exceeded when calling POST /api/services/v2/{id}.");
            case "unknown":
                throw new errors.BridgeApiError({
                    message: _response.error.errorMessage,
                    rawResponse: _response.rawResponse,
