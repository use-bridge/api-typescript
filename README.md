...
```sh
npm i -s @usebridge/api
```

## Reference

A full reference for this library is available [here](https://github.com/use-bridge/api/blob/HEAD/./reference.md).
...
const request: BridgeApi.ConsentVersionsListV1Request = {
    ...
};
```
...
try {
    await client.consent.createPatientConsent(...);
} catch (err) {
...
```typescript
const response = await client.consent.createPatientConsent(..., {
    headers: {
...
```typescript
const { data, rawResponse } = await client.consent.createPatientConsent(...).withRawResponse();
